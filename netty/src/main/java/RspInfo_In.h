...
#ifndef _TFZQ_RSPINFO_IN_H_
#define _TFZQ_RSPINFO_IN_H_
...
#pragma GCC diagnostic pop
    ...
    static SBE_CONSTEXPR const std::uint32_t errorIDNullValue()
    {
        return SBE_NULLVALUE_UINT32;
    }

    static SBE_CONSTEXPR const std::uint32_t errorIDMinValue()
    {
        return 0;
    }

    static SBE_CONSTEXPR const std::uint32_t errorIDMaxValue()
    {
        return 4294967293;
    }
    ...
    std::uint32_t errorID(void) const
    {
        return SBE_LITTLE_ENDIAN_ENCODE_32(*((std::uint32_t *)(m_buffer + m_offset + 0)));
    }

    RspInfo_In &errorID(const std::uint32_t value)
    {
